...
dc=read.csv("./DUXOE_up_major_ZGA_siHN.csv",header=T,check.names=F)
rownames(dc) <- dc$gene
...
pdf(file="./DUXOE_up_major_ZGA_DUXOEASO_embryo_siHN.pdf",width =5,height =8)
draw(p1+p2+p3)
dev.off()
...
dc=read.csv("./DUXOE_up_minor_ZGA_siHN.csv",header=T,check.names=F)
rownames(dc) <- dc$gene
dc <- dc[,2:length(colnames(dc))]
dc <- as.matrix(dc)
...
p3=Heatmap(dc_scaled,row_names_gp = gpar(fontsize = 15),show_row_names = F,
           column_names_gp = gpar(fontsize = 15),cluster_columns = FALSE,col=col_fun_2,cluster_rows = FALSE)+ rowAnnotation(link = anno_mark(at = index, 
                                                                                                                                             labels = labs, labels_gp = gpar(fontsize = 10)))
pdf(file="./DUXOE_up_minor_ZGA_DUXOEASO_embryo_siHN.pdf",width =5,height =8)
draw(p1+p2+p3)
dev.off()
